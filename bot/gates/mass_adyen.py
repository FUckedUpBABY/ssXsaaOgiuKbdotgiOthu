...
from FUNC.GATES.massfunc import *
import threading
import asyncio
from FUNC.GATES.ADYEN.func.adyen import *

@Client.on_message(filters.command("massad", [".", "/"]))
...
𝐃𝐎𝐍'𝐓 𝐓𝐑𝐘 𝐁𝐄𝐅𝐎𝐑𝐄 {after} 𝗦𝗘𝗖𝗢𝗡𝗗𝗦
            """
                        ...
                        if len(all_cards) > 9 and role == "FREE":
                            resp = "𝗦𝗢𝗥𝗥𝗬 𝐅𝐫𝐞𝐞 𝐁𝐨𝐳𝐨𝐬 𝐚𝐫𝐞 𝐋𝐢𝐦𝐢𝐭𝐞𝐝 𝐭𝐨 8 𝗖𝗖 ❌"
                            free = await Client.edit_message_text(
                                message.chat.id, stchk.id, resp)
                        elif len(
                                all_cards
                        ) > 26 and role == 'PREMIUM' and plan == "N/A":
                            resp = "𝐏𝐑𝐄𝐌𝐈𝐔𝐌 𝐁𝐎𝐙𝐎𝐒 𝐂𝐀𝐍 𝐂𝐇𝐄𝐂𝐊 𝐔𝐏𝐓𝐎 15 𝗖𝗖 ❌"
                            ...
                            if not len_cards:
                                ...
                                text = f"""
<b>𝐆𝐀𝐓𝐄 ⟿ 𝐌𝐀𝐒𝐒 𝐀𝐔𝐓𝐇 (𝐀𝐃𝐘𝐄𝐍)</b> \n
"""
                                amt = 0
                                with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                                    futures = []
                                    for i in cards:
                                        future = executor.submit(getmassad,i,user_id)
